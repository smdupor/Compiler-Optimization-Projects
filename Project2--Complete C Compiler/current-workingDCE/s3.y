...
/* macros  */
%token HALT IN OUT GETC PUTS PUTSP 
...
void print_op(struct operand op)
{
  enum op_type t =  op.t;
  switch(t) {
  case op_reg:
...
void print_inst(inst_t i)
{
  ...
  case OP_STR:
  case OP_SUB:
    print_op(i->ops[0]); printf(", ");
    print_op(i->ops[1]); printf(", ");
    print_op(i->ops[2]); 
    break;
    /* 2 operands */
  case OP_BR:
