...
void print_regset(int set, int printR);
...
void calculate_regsets(){
	...
	calculate_genset();
	vbmode{ printf("done.\n"); 
			print_regset(0, FALSE);
	}

	vbmode{ printf("  Calculating kill[]..."); }
	calculate_killset();
	vbmode{ printf("done.\n"); 
			print_regset(1, FALSE);
	}
 ...
 *			case 6: livein
 *			case 7: liveout
 *		printR: a boolean wheather to print values as registers.
...
void setup_arrays(){
	...
	while(curr){
		...
		if((curr->gen = (int *)malloc(NUM_INSTR*sizeof(int))) == NULL){
		    vbmode{ printf("Could not allocate memory!\n"); }
		}
		if((curr->kill = (int *)malloc(NUM_INSTR*sizeof(int))) == NULL){
		    vbmode{ printf("Could not allocate memory!\n"); }
		}
		if((curr->defin = (int *)malloc(NUM_INSTR*sizeof(int))) == NULL){
		    vbmode{ printf("Could not allocate memory!\n"); }
		}
		if((curr->defout = (int *)malloc(NUM_INSTR*sizeof(int))) == NULL){
		    vbmode{ printf("Could not allocate memory!\n"); }
		}
